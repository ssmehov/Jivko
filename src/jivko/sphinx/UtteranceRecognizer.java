 ...
 * buffers all audio data until a DataEndSignal occurs.
 *
...
public class UtteranceRecognizer extends BaseDataProcessor {
  ...
  public void newProperties(PropertySheet ps) throws PropertyException {
    ...
    outFileNamePattern = ps.getString(edu.cmu.sphinx.frontend.util.WavWriter.PROP_OUT_FILE_NAME_PATTERN);
  ...
  /**
   * Converts a big-endian byte array into an array of doubles. Each consecutive
   * bytes in the byte array are converted into a double, and becomes the next
   * element in the double array. The size of the returned array is
   * (length/bytesPerValue). Currently, only 1 byte (8-bit) or 2 bytes (16-bit)
